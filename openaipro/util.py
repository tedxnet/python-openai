...
def get_object_classes():
    ...
    from openaipro.object_classes import OBJECT_CLASSES
...
def convert_to_openai_object(
    ...
    if isinstance(resp, openaipro.openai_response.OpenAIResponse):
    ...
    if plain_old_data:
        ...
        resp, openaipro.openai_object.OpenAIObject
    ):
        resp = resp.copy()
        klass_name = resp.get("object")
        if isinstance(klass_name, str):
            klass = get_object_classes().get(
                klass_name, openaipro.openai_object.OpenAIObject
            )
        else:
            klass = openaipro.openai_object.OpenAIObject
...
def default_api_key() -> str:
    if openaipro.api_key_path:
        with open(openaipro.api_key_path, "rt") as k:
            api_key = k.read().strip()
            if not api_key.startswith("sk-"):
                raise ValueError(f"Malformed API key in {openaipro.api_key_path}.")
            return api_key
    elif openaipro.api_key is not None:
        return openaipro.api_key
    else:
        raise openaipro.error.AuthenticationError(
            "No API key provided. You can set your API key in code using 'openaipro.api_key = <API-KEY>', or you can set the environment variable OPENAI_API_KEY=<API-KEY>). If your API key is stored in a file, you can point the openai module at it with 'openaipro.api_key_path = <PATH>'. You can generate API keys in the OpenAI web interface. See https://onboard.openai.com for details, or email support@openai.com if you have any questions."
        )
