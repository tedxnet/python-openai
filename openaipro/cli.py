...
import openai
from openaipro.upload_progress import BufferReader
from openaipro.validators import (
...
class Engine:
    @classmethod
    def get(cls, args):
        engine = openaipro.Engine.retrieve(id=args.id)
        display(engine)

    @classmethod
    def update(cls, args):
        engine = openaipro.Engine.modify(args.id, replicas=args.replicas)
        display(engine)
    ...
    def generate(cls, args):
        ...
        resp = openaipro.Engine(id=args.id).generate(
...
class Completion:
    ...
    def create(cls, args):
        ...
        resp = openaipro.Completion.create(
            engine=args.engine,
            model=args.model,
            n=args.n,
...
class Deployment:
    @classmethod
    def get(cls, args):
        resp = openaipro.Deployment.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        model = openaipro.Deployment.delete(args.id)
        print(model)

    @classmethod
    def list(cls, args):
        models = openaipro.Deployment.list()
        print(models)

    @classmethod
    def create(cls, args):
        models = openaipro.Deployment.create(model=args.model, scale_settings={"scale_type": args.scale_type})
        print(models)
...
class Model:
    @classmethod
    def get(cls, args):
        resp = openaipro.Model.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        model = openaipro.Model.delete(args.id)
        print(model)

    @classmethod
    def list(cls, args):
        models = openaipro.Model.list()
        print(models)
...
class File:
    ...
    @classmethod
    def get(cls, args):
        resp = openaipro.File.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        file = openaipro.File.delete(args.id)
        print(file)

    @classmethod
    def list(cls, args):
        file = openaipro.File.list()
        print(file)
...
class Image:
    ...
    def create_variation(cls, args):
        with open(args.image, "rb") as file_reader:
            buffer_reader = BufferReader(file_reader.read(), desc="Upload progress")
        resp = openaipro.Image.create_variation(
            image=buffer_reader,
            size=args.size,
            n=args.num_images,
            response_format=args.response_format,
    ...
    def create_edit(cls, args):
        ...
        resp = openaipro.Image.create_edit(
            image=image_reader,
            mask=mask_reader,
            prompt=args.prompt,
            size=args.size,
...
class FineTune:
    ...
    def _maybe_upload_file(
        ...
        if check_if_file_exists:
            bytes = len(content)
            matching_files = openaipro.File.find_matching_files(
    ...
    def _get_or_upload(cls, file, check_if_file_exists=True):
        try:
            ...
            openaipro.File.retrieve(file)
            return file
        except openaipro.error.InvalidRequestError:
            pass
    ...
    def create(cls, args):
        ...
        resp = openaipro.FineTune.create(**create_args)
    ...
    @classmethod
    def get(cls, args):
        resp = openaipro.FineTune.retrieve(id=args.id)
        print(resp)

    @classmethod
    def results(cls, args):
        fine_tune = openaipro.FineTune.retrieve(id=args.id)
        if "result_files" not in fine_tune or len(fine_tune["result_files"]) == 0:
            raise openaipro.error.InvalidRequestError(
                f"No results file available for fine-tune {args.id}", "id"
            )
        result_file = openaipro.FineTune.retrieve(id=args.id)["result_files"][0]
        resp = openaipro.File.download(id=result_file["id"])
        print(resp.decode("utf-8"))

    @classmethod
    def events(cls, args):
        if args.stream:
            raise openaipro.error.OpenAIError(
                message=(
        ...
        resp = openaipro.FineTune.list_events(id=args.id)  # type: ignore
        print(resp)
    ...
    def _stream_events(cls, job_id):
        def signal_handler(sig, frame):
            status = openaipro.FineTune.retrieve(job_id).status
            sys.stdout.write(
        ...
        events = openaipro.FineTune.stream_events(job_id)
        ...
        resp = openaipro.FineTune.retrieve(id=job_id)
        status = resp["status"]
        if status == "succeeded":
    ...
    @classmethod
    def cancel(cls, args):
        resp = openaipro.FineTune.cancel(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        resp = openaipro.FineTune.delete(sid=args.id)
        print(resp)
...
class WandbLogger:
    @classmethod
    def sync(cls, args):
        import openaipro.wandb_logger

        resp = openaipro.wandb_logger.WandbLogger.sync(
            id=args.id,
